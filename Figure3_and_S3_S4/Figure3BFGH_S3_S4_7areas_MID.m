...
mid_dir = sprintf('%s/MID', paper_data_dir);
...
for s=1:length(pt_subjs)
    ...
    wts = zeros(total_chans, 4800); % Have to do this to avoid indexing issues
    for ch=1:length(mid_data.data)
        wts(mid_elecs.elec_nums(ch),:) = mid_data.data(ch).MID1{1}(:);
    end

    %sig_inds = find(vcorrs>0.1);
    sig_inds = mid_elecs.elec_nums;
...
axis tight;
print_quality_fig(gcf,sprintf('%s/Figure3H_MID.eps',figDir),8,3,3,'inches','epsc');
%%
